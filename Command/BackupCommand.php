...
class BackupCommand extends StorageCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $dumpDatabaseStructureCommand = 'mysql -u '.$this->connection->getUsername().' --password='.$this->connection->getPassword().' -h '.$this->connection->getHost().' '.$this->connection->getDatabase().' -e \'SHOW TABLES WHERE `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "application_logs_%" AND `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "PLEASE_DELETE%"\' | grep -v Tables_in | xargs mysqldump'.($columnStatisticsSupported ? ' --column-statistics=0' : '').' --no-data '.($purgeIdSupported ? '--set-gtid-purged=OFF' : '').' --skip-triggers --routines -u '.$this->connection->getUsername().' --password='.$this->connection->getPassword().' -h '.$this->connection->getHost().' '.$this->connection->getDatabase().' -r '.$tmpDatabaseDump;

        $dumpDatabaseDataCommand = 'mysql -u '.$this->connection->getUsername().' --password='.$this->connection->getPassword().' -h '.$this->connection->getHost().' '.$this->connection->getDatabase().' -e \'SHOW TABLES WHERE `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "application_logs_%" AND `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "PLEASE_DELETE%" AND `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "application_logs" AND `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "edit_lock" AND `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "cache" AND `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "cache_tags" AND `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "email_log" AND `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "http_error_log" AND `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "search_backend_data" AND `Tables_in_'.$this->connection->getDatabase().'` NOT LIKE "tmp_store"\' | grep -v Tables_in | xargs mysqldump'.($columnStatisticsSupported ? ' --column-statistics=0' : '').' --no-create-info --skip-triggers '.($purgeIdSupported ? '--set-gtid-purged=OFF' : '').' -u '.$this->connection->getUsername().' --password='.$this->connection->getPassword().' -h '.$this->connection->getHost().' '.$this->connection->getDatabase().' >> '.$tmpDatabaseDump.' && sed -i \'/^\/\*!50013 /d\' '.$tmpDatabaseDump.' && sed -i \'s/ALGORITHM.*VIEW/VIEW/g\' '.$tmpDatabaseDump.' && sed -i \'s/\sDEFINER=`[^`]*`@`[^`]*`//g\' '.$tmpDatabaseDump;
